...
class PrivateWorkplaceTest(TestViewsSetUp):
    def test_retrieve_workspace(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        self.assertQuerySetEqual(
            response.context["workplace_list"],
            workplaces
        )
    ...
    def test_search_workspace_by_name(self) -> None:
        response = self.client.get(
            WORKPLACE_URL,
            {"name": self.workplace_search}
        )
        workplaces = Workplace.objects.filter(
            name__icontains=self.workplace_search
        )
        self.assertEqual(response.status_code, 200)
        self.assertQuerySetEqual(
            response.context["workplace_list"],
...
class PrivateMaterialTest(TestViewsSetUp):
    def test_retrieve_material(self) -> None:
        ...
        self.assertQuerySetEqual(
            response.context["materials_list"],
            expected_materials
        )
        self.assertTemplateUsed(
            response,
...
class PrivatePrinterTest(TestViewsSetUp):
    def test_retrieve_printer(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        self.assertQuerySetEqual(
            response.context["printers_list"],
            expected_printers
        )
...
class PrivatePrintQueueTest(TestViewsSetUp):
    def test_retrieve_queue(self) -> None:
        ...
        self.assertEqual(response.status_code, 200)
        self.assertQuerySetEqual(
            response.context["printqueue_list"],
            expected_queues
        )
...
class ChangeOrderStatusTest(TestViewsSetUp):
    ...
    def target_url(pk: int) -> str:
        return reverse(
            "production:change-order-status",
            args=[pk]
        )

    def setUp(self):
        super().setUp()
        self.printer1.materials.add(
            self.material1
        )
    ...
    def test_change_status_to_ready_with_queue(self):
        ...
        self.assertEqual(
            self.print_queue.status, PrintQueue.READY_TO_PRINT
        )
