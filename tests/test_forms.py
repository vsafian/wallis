...
class FormsTest(TestCase):
    ...
    def phone_number_form_is_valid(number: str) -> bool:
        return WorkerPhoneNumberForm(data={"phone_number": number}).is_valid()

    def test_worker_creation_form_is_valid(self) -> None:
        self.assertTrue(self.form.is_valid())

    def test_phone_number_is_valid(self) -> None:
        self.assertTrue(self.phone_number_form_is_valid("+380999999999"))
    ...
    def test_phone_number_not_more_than_13(self) -> None:
        self.assertFalse(self.phone_number_form_is_valid("+3809999999999999999"))

    def test_phone_number_with_invalid_characters(self) -> None:
        self.assertFalse(self.phone_number_form_is_valid("+380ABCDEF123"))
...
class TestWorkplaceForm(TestItems):
    ...
    def setUp(self):
        ...
        self.instance_form_data = {"name": self.instance.name}
    ...
    def test_form_initial_context(self):
        ...
        self.assertListEqual(self.printers, list(initial_context["printers"]))
        self.assertListEqual(workers, list(initial_context["workers"]))
    ...
    def test_form_printers_queryset_while_object_was_created(self) -> None:
        ...
        self.assertQuerySetEqual(
            expected_queryset, form.fields.get("printers").queryset.all()
        )

    def test_form_printers_queryset_when_create_new_object(self) -> None:
        expected_queryset = Printer.objects.filter(workplace=None)
        form = WorkplaceForm(data=self.new_object_form_data)
        self.assertQuerySetEqual(
            expected_queryset, form.fields.get("printers").queryset.all()
        )
    ...
    def test_printers_assigned_to_workplace(self) -> None:
        self.new_object_form_data.update(
            {"printers": Printer.objects.filter(workplace=None)}
        )
        form = WorkplaceForm(data=self.new_object_form_data)
        instance = form.save()
        instance_printers = instance.printers.all()
        expected_printers = Printer.objects.filter(workplace=instance)
        free_printers = Printer.objects.filter(workplace=None)
        self.assertQuerySetEqual(expected_printers, instance_printers)
    ...
    def test_printers_remove_from_workplace(self):
        ...
        self.instance_form_data.update({"printers": []})
...
class TestPrintQueueCreateForm(TestItems):
    ...
    def test_form_valid_with_correct_data(self) -> None:
        ...
        form = PrintQueueCreateForm(data=form_data, cached_workplace=self.workplace)
        self.assertTrue(form.is_valid())

    def test_material_queryset_is_filtered_by_workplace_printers(self) -> None:
        form = PrintQueueCreateForm(cached_workplace=self.workplace)
    ...
    def test_orders_queryset_is_filtered_by_workplace_materials(self) -> None:
        ...
        expected_orders = Order.objects.filter(material__in=materials)
    ...
    def test_orders_queryset_is_filtered_by_selected_material(self) -> None:
        form = PrintQueueCreateForm(
            data={"material": self.material1}, cached_workplace=self.workplace
        )
        form.is_valid()
        orders_queryset = form.get_field_queryset("orders")
    ...
    def test_form_saves_correctly(self):
        ...
        form_data = {"material": self.material1, "orders": orders}
        form = PrintQueueCreateForm(data=form_data, cached_workplace=self.workplace)
        self.assertTrue(form.is_valid())
        instance = form.save()
...
class TestPrintQueueUpdateForm(TestItems):
    ...
    def test_form_initial_data_is_correct(self):
        ...
        self.assertEqual(form.initial["workplace"], self.workplace1)
        self.assertEqual(form.initial["material"], self.material1)
        self.assertEqual(set(form.initial["orders"]), set(self.orders))
    ...
    def test_form_is_valid_with_correct_data(self):
        ...
        form = PrintQueueUpdateForm(data=form_data, cached_instance=self.print_queue)
        self.assertTrue(form.is_valid())
    ...
    def test_orders_are_updated_correctly_on_save(self):
        ...
        self.assertEqual(set(instance.orders.all()), set(self.orders))
