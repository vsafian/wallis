    ...
    phone_number = models.CharField(
        ...
        validators=[
            RegexValidator(
                regex=r"^\+380\d{9}$",
                message=(
                    "Phone number " "must be entered in the format: " "<+380999999999>!"
                ),
                code="invalid_number",
            ),
        ],
...
class Printer(
    ...
    materials = models.ManyToManyField(Material, related_name="printers")
    workplace = models.ForeignKey(
        Workplace,
...
class PrintQueue(PrintStatusMixin, models.Model, ModelAbsoluteUrlMixin):
    status = models.CharField(
        max_length=50,
...
class Order(
    ...
    performing_time = models.DateTimeField(null=True, blank=True)
    image_name = models.CharField(max_length=255)
    material = models.ForeignKey(
        Material, related_name="orders", on_delete=models.CASCADE
    )
    ...
    def square_meters(self) -> float:
        return round(self.width * self.height / 10000, 2)
    ...
    def narrow_tile_width(self) -> float:
        return round((self.width / self.tiles_count) * 10, 2)
